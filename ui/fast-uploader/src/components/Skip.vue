<template>
  <div id="page">
    ...
    <footer>
      <p>前端转载此例子进行修改和使用，出处为：<a href="https://www.helloweba.net/javascript/632.html">www.helloweba.net</a></p>
      <p class="hidden"></p>
    </footer>
  </div>
</template>
...
<script>
  ...
  export default {
    data() {
      return {
        options: {
          target: 'http://127.0.0.1:8000/upload_file/',
          chunkSize: 1024 * 1024 * 5,  //2MB
    ...
    methods: {
      ...
      onFileSuccess(rootFile, file, response, chunk) {
        ...
        if (resp.code === 0 && resp.merge === true) {
          axios.post('http://127.0.0.1:8000/merge_files/', {
            filename: file.name,
